...
def lbdm_graph(track: mido.MidiTrack, ticks_per_beat: int = 1024):
    ...
    barwidth = 0.3
    plt.plot(np.array(range(len(profile))), profile, label="LBDM overall boundary strength", linewidth=2, color="blue")
    plt.scatter(np.array(range(len(profile))), profile, color="blue")
